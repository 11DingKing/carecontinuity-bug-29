...
func TestSubscriptionReplayAfterReconnect(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		event := &domain.Event{
			Type: domain.EventMailLoaded, BusinessKey: "mail-" + itoa(i),
...
func TestSubscriptionSlowConsumerEvicted(t *testing.T) {
	...
	for i := 0; i < subscriberBufferSize+10; i++ {
		event := &domain.Event{
			Type: domain.EventMailLoaded, BusinessKey: "mail-ev-" + itoa(i),
			ShardID: "2026-08-19_R001", Payload: "{}", CreatedAt: clock.Now(),
		}
...
func TestSubscriptionPersistCheckpoint(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		event := &domain.Event{
			Type: domain.EventMailInTransit, BusinessKey: "mail-cp-" + itoa(i),
			ShardID: "2026-08-19_R001", Payload: "{}", CreatedAt: clock.Now(),
		}
		require.NoError(t, bus.Publish(ctx, event))
		clock.Advance(1e9)
...
func TestSubscriptionOrderGuarantee(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		event := &domain.Event{
			Type: domain.EventMailLoaded, BusinessKey: "mail-ord-" + itoa(i),
			ShardID: "2026-08-19_R001", Payload: "{}", CreatedAt: clock.Now(),
		}
		require.NoError(t, bus.Publish(ctx, event))
		clock.Advance(1e9)
