...
func sharedRecordLedger(ctx context.Context, store storage.Store, clock domain.Clock, shardID, formNo, mailNo, responsible, entryType, prevState, nextState string) {
	date, routeID := domain.SplitShardID(shardID)
	entry := &domain.LedgerEntry{
		ID: uuid.NewString(), Date: date, RouteID: routeID, VolumeNo: date + "_" + routeID,
		FormNo: formNo, MailNo: mailNo, Responsible: responsible, EntryType: entryType,
	...
	store.LedgerRepo().Save(ctx, entry)
}
