...
func applyEnvOverrides(cfg *Config) {
	if v := os.Getenv("CARECONTINUITY_PORT"); v != "" {
		var port int
		fmt.Sscanf(v, "%d", &port)
		if port > 0 {
			cfg.Port = port
	...
	if v := os.Getenv("CARECONTINUITY_DATA_DIR"); v != "" {
		cfg.DataDir = v
	}
	if v := os.Getenv("CARECONTINUITY_LOG_LEVEL"); v != "" {
		cfg.LogLevel = v
	}
	if v := os.Getenv("CARECONTINUITY_READ_TIMEOUT"); v != "" {
		if d, err := time.ParseDuration(v); err == nil {
			cfg.ReadTimeout = d
		}
	}
	if v := os.Getenv("CARECONTINUITY_WRITE_TIMEOUT"); v != "" {
		if d, err := time.ParseDuration(v); err == nil {
			cfg.WriteTimeout = d
		}
	}
	if v := os.Getenv("CARECONTINUITY_REPLAY_WINDOW_HOURS"); v != "" {
		fmt.Sscanf(v, "%d", &cfg.ReplayWindowHours)
	}
	if v := os.Getenv("CARECONTINUITY_SIGNOFF_TIMEOUT_HOURS"); v != "" {
