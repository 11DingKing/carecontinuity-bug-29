...
func scanLedger(s rowScanner) (*domain.LedgerEntry, error) {
	var e domain.LedgerEntry
	var createdAt string
	if err := s.Scan(&e.ID, &e.Date, &e.RouteID, &e.VolumeNo, &e.FormNo, &e.EntryType,
		&e.MailNo, &e.Responsible, &e.Description, &e.PrevState, &e.NextState,
		&createdAt, &e.ShardID, &e.DataVersion); err != nil {
		return nil, wrapScanErr(err, "ledger")
	}
...
func scanBatch(s rowScanner) (*domain.BatchRecord, error) {
	...
	if err := s.Scan(&b.ID, &b.VehicleNo, &b.Date, &b.RouteID, &b.State, &b.TotalCount,
...
func scanBatchItem(s rowScanner) (*domain.BatchItem, error) {
	...
	var createdAt, updatedAt string
	if err := s.Scan(&item.ID, &item.BatchID, &item.MailID, &item.MailNo, &item.State,
...
const shardMetaColumns = "shard_id, date, route_id, file_path, record_count, checksum, status, data_version, created_at, updated_at"
	...
	_, err := ex.ExecContext(ctx,
		`INSERT INTO shard_meta (`+shardMetaColumns+`)
		 ...
		 ON CONFLICT(shard_id) DO UPDATE SET
			date=excluded.date, route_id=excluded.route_id, file_path=excluded.file_path,
		...
		m.ShardID, m.Date, m.RouteID, m.FilePath, m.RecordCount, m.Checksum,
