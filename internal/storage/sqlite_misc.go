...
func (r *shardMetaRepo) Get(ctx context.Context, shardID string) (*ShardMeta, error) {
	row := r.store.db.QueryRowContext(ctx,
		`SELECT shard_id, date, route_id, file_path, record_count, checksum, status, data_version, created_at, updated_at
...
func scanShardMeta(s rowScanner) (*ShardMeta, error) {
	...
	err := s.Scan(&m.ShardID, &m.Date, &m.RouteID, &m.FilePath, &m.RecordCount,
