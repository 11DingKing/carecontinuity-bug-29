...
func TestHazardLifecycleRequiresEvidence(t *testing.T) {
	...
	r, _ := a.Login(ctx, "regional_coordinator", "regional-coordinator-demo")
	o, _ := a.Login(ctx, "community_provider", "community-provider-demo")
	i, _ := a.Login(ctx, "lab_coordinator", "lab-coordinator-demo")
	if _, err := svc.CreateStation(ctx, r.ID, Station{ID: "st-1", Name: "人民路站", OperatorID: "u-community-provider"}); err != nil {
		t.Fatal(err)
	}
