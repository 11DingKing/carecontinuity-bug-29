...
func (s *Service) CreateStation(ctx context.Context, token string, station Station) (Station, error) {
	if _, err := s.auth.RequireRole(ctx, token, auth.RoleRegionalCoordinator, auth.RoleNationalAdmin); err != nil {
		return Station{}, err
	}
...
func (s *Service) ReportHazard(ctx context.Context, token string, hazard Hazard) (Hazard, error) {
	session, err := s.auth.RequireRole(ctx, token, auth.RoleLabCoordinator, auth.RoleNationalAdmin)
	if err != nil {
		return Hazard{}, err
	}
...
func (s *Service) RectifyHazard(ctx context.Context, token, id, evidence string) error {
	session, err := s.auth.RequireRole(ctx, token, auth.RoleCommunityProvider)
	if err != nil {
		return err
	}
	if evidence == "" {
...
func (s *Service) VerifyHazard(ctx context.Context, token, id, evidence string) error {
	if _, err := s.auth.RequireRole(ctx, token, auth.RoleLabCoordinator, auth.RoleRegionalCoordinator); err != nil {
		return err
	}
	if evidence == "" {
		return ErrMissingEvidence
