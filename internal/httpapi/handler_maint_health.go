...
func (s *Server) VerifyData(w http.ResponseWriter, r *http.Request) {
	report, err := s.maintSvc.Verify(r.Context())
	respondJSON(w, report, err)
}
...
func (s *Server) BatchCheckMails(w http.ResponseWriter, r *http.Request) {
	var req BatchCheckRequest
	if err := decodeJSON(r, &req); err != nil {
		writeError(w, err)
		return
	}
	result, err := s.maintSvc.BatchVerifyMailItems(r.Context(), req.MailIDs)
...
func (s *Server) Readyz(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := s.store.MailItemRepo().Get(r.Context(), "readyz-probe"); err != nil {
