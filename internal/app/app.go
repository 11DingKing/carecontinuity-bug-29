...
import (
	...
	"carecontinuity/internal/audit"
	"carecontinuity/internal/auth"
	"carecontinuity/internal/circuit"
	"carecontinuity/internal/config"
	"carecontinuity/internal/domain"
	"carecontinuity/internal/httpapi"
	"carecontinuity/internal/prevention"
	"carecontinuity/internal/ratelimit"
	"carecontinuity/internal/scheduler"
	"carecontinuity/internal/service"
	"carecontinuity/internal/sla"
	"carecontinuity/internal/storage"
)
...
func New(ctx context.Context, cfg *config.Config, clock domain.Clock) (*App, error) {
	...
	handSvc := service.NewHandoverService(store, clock, eventBus)
	dispSvc := service.NewDispositionService(store, clock, eventBus).
		WithBreaker(circuit.New(circuit.Config{
			Name: "disposition_execute", MaxRequests: 3, Timeout: 30 * time.Second,
	...
	overdueSvc := service.NewOverdueService(store, clock, cfg.SignoffTimeout()).
		WithSLA(sla.NewRuleSet(cfg.SignoffTimeoutHours))
...
func (a *App) Start(ctx context.Context) error {
	...
	srv := httpapi.NewServer(
		...
		a.subSvc, a.ledgerSvc, a.importSvc, a.overdueSvc, a.maintSvc, a.scheduler, a.authSvc,
	)
	srv.SetPreventionService(prevention.NewService(a.store, a.authSvc, a.clock.Now))
...
func (a *App) HandoverService() *service.HandoverService         { return a.handSvc }
func (a *App) DispositionService() *service.DispositionService   { return a.dispSvc }
...
func (a *App) LedgerService() *service.LedgerService             { return a.ledgerSvc }
